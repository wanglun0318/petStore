...
from com.petstore.dao.account_dao import AccountDao
...
class LoginFrame(MyFrame):
    def __init__(self):
        ...
        # 创建界面控件
        accountid_st = wx.StaticText(self.contentpanel, label='帐号：')
    ...
    def okb_btn_onclick(self, event):
        ...
        if account is not None and account['password'] == password:
            ...
            print('登录失败')
            dlg = wx.MessageDialog(self, '您输入的帐号或者密码有误，请重新输入。', '登录失败', wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
